...
wp_enqueue_script("ld-groups-js");
...
foreach($collection_details["courses"] as $course){
	...
	/*
	echo "<pre>";
		print_r(["course_id"=>$course["course_id"], "status" => $course["status"], "active_slide"=>$active_slide]);
	echo "</pre>";
	*/

	$group_courses[] = $course["course_id"];
}


?>
<a href="/course-library/" class="btn-back" ><img src="<?=get_stylesheet_directory_uri();?>/assets/img/groups/group-back.png" alt="Back Button"></a>
<div class="g-actions">
	<img src="<?=get_stylesheet_directory_uri();?>/assets/img/groups/groupcoin.png" alt="Points">
	<div class="gcoins"><svg viewBox="0 0 103 13" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M93.317.63H9.691C4.57.63.424 5.025.424 10.452v2.121c0-5.427 4.147-9.823 9.267-9.823h83.618c5.12 0 9.267 4.396 9.267 9.823v-2.121c0-5.427-4.147-9.824-9.267-9.824h.008Z" fill="#000" fill-opacity=".15" style="mix-blend-mode:multiply"/></svg><span><?=$user_info->data->points?></span></div>
</div>
<div class="<?php echo esc_attr( learndash_the_wrapper_class() ); ?>" testgroup>
	...
	if ( $has_group_content && $show_group_content ) :
		...
		if ( ( defined( 'LEARNDASH_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === LEARNDASH_TEMPLATE_CONTENT_METHOD ) ) {
			...
			?>
			<div class="ld-item-list ld-lesson-list" testlessonlist>
				<div class="ld-section-heading">
					...
					<div class="ld-item-list-actions " testlistactions data-ld-expand-list="true">
...
<script>
	jQuery(document).ready(function ($) {
		$('.ld-item-list-items').slick({
			...
			dots: true,
			variableWidth:true,
            prevArrow:'<button type="button" class="slick-prev"><img src="<?=get_stylesheet_directory_uri();?>/assets/img/groups/slick-prevbtn.png" /></button>',
            nextArrow:'<button type="button" class="slick-next"><img src="<?=get_stylesheet_directory_uri();?>/assets/img/groups/slick-nextbtn.png" /></button>'
		}).on('afterChange', function(event, slick, currentSlide, nextSlide){
			let bg = $('.slick-current .ld-item-list-item-preview').data('img');
			if (bg) {
				$('.single-groups #page').css('background-image',`url(${bg})`)
			}else {
				$('.single-groups #page').removeAttr("style")
			}
			console.log(bg)
		});

		<?php 
		if(!empty($active_slide)){
			?>
			$('.ld-item-list-items').slick('slickGoTo', <?=$active_slide?>);
			<?php
		}
		?>
	})
