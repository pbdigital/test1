...
let ChooseAvatar = {
    ...
    api: {
        avatarItems: async (e) => {
            return new Promise((resolve, reject) => {
                ...
                if(ChooseAvatar.apiResponseAvatarItems ==""){
                    $.ajax({
                        ...
                        success: d => {
                            ChooseAvatar.apiResponseAvatarItems = d;
                            resolve(ChooseAvatar.apiResponseAvatarItems);
                        },
                        error: (d) => {
    ...
    getAvatarItems: (e) => {
        ...
        ChooseAvatar.api.avatarItems(e)
            .then( d => { 
                ChooseAvatar.avatarItems = d.items;
                tpl = ``;
                ...
                d.items.map((e) => {
                    ...
                    if(d.sub_categories.length <=0){
                        ...
                        if(ChooseAvatar.subTypeSelected == ""){
                            // get the first subcategory
                            ChooseAvatar.subTypeSelected = d.sub_categories[0].term_id;
                        }
                        
                        if(e.terms.length > 0){
                            e.terms.map( term => {
                                //console.log("term.term_id == ChooseAvatar.subTypeSelected", term.term_id , ChooseAvatar.subTypeSelected)
                                // check if the SubCategory selected matches with the item, IF yes show
                                if(term.term_id == ChooseAvatar.subTypeSelected  ){
                                    show = true;
                ...
                if(d.sub_categories.length <=0){
                    ...
                    d.sub_categories.map( subCat => {
                        ...
                        if(subCat.term_id == ChooseAvatar.subTypeSelected){
                            isActive = "active";
        ...
        $.ajax({
            ...
            success: (d) => {
                ...
                let heroFemale = `
                    ...
                    <svg width="135" height="118" viewBox="0 0 135 118" fill="none" xmlns="http://www.w3.org/2000/svg" class="hair-back">
    ...
    saveAvatar: () => {
        console.log(ChooseAvatar.selectedAvatars)
        $.ajax({
            url: `${safarObject.apiBaseurl}/user/avatar`,
            data: {
                group : ChooseAvatar.selectedAvatars.group,
                skin_color : ChooseAvatar.selectedAvatars.skinColor,
                hair_style : ChooseAvatar.selectedAvatars.hairStyle,
                hair_color : ChooseAvatar.selectedAvatars.hairColor,
                avatar_html: $(".avatars-hero").html(),
                gender: ChooseAvatar.selectedAvatars.gender
            },
            type: "post",
            headers: {
                "X-WP-Nonce": safarObject.wpnonce
            ...
            success: (d) => {
                // Check if the 'redirect' parameter has a value
                const redirectValue = getURLParameter('redirect');
                if (redirectValue) {
    ...
    init : () => {
        ...
        ChooseAvatar.getAvatarItems({"type": ChooseAvatar.typeSelected, "taxonomy": ChooseAvatar.taxonomySelected });
        ChooseAvatar.getUserEquippedGears();
...
$(document).on("click", ".select-group-group label", e => {
    ...
    ChooseAvatar.selectedAvatars.group =  $(e.currentTarget).find("input").val();
...
$(document).on("click",".avatars-categories__list-item.item-avatar", e => {
    ...
    switch(subType){
        case "skin-color":
            ...
            ChooseAvatar.selectedAvatars.skinColor = itemId
        break;
        ...
        case "hair-color":
            ...
            ChooseAvatar.selectedAvatars.hairColor = itemId
        break;

        case "hairstyle":
            ChooseAvatar.selectedAvatars.hairStyle = itemId
            ChooseAvatar.avatarItems.map( e => {
                if(e.ID == itemId){
                    ...
                    if(ChooseAvatar.selectedAvatars.hairColor != ""){
                        ChooseAvatar.avatarItems.map( eachHaircolor => {
                            if(eachHaircolor.ID == ChooseAvatar.selectedAvatars.hairColor){
...
$(document).on("click",".avatars-categories__list-sub-categories button", e => {
    ChooseAvatar.subTypeSelected = $(e.currentTarget).attr("data-termid");
    ChooseAvatar.getAvatarItems({"type": ChooseAvatar.typeSelected, "taxonomy": ChooseAvatar.taxonomySelected });
});

$(document).on("click",".btn-select-avatar", e => {
    ChooseAvatar.saveAvatar();
});

$(document).on("click", ".btn-select-gender", e => {
    $(e.currentTarget).fadeTo("fast",.3);
    ChooseAvatar.selectedAvatars.gender = $(".select-gender:checked").val();

    ChooseAvatar.saveGender()
        .then( e => {
            ChooseAvatar.apiResponseAvatarItems = "";
            $(".choose-avatar-inner .tab").removeClass("active");
            $(".choose-avatar-inner .tab.color-select").addClass("active");
