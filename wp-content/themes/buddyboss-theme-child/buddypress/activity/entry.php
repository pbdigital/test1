...
<li class="<?php bp_activity_css_class(); ?>" id="activity-<?php bp_activity_id(); ?>" data-bp-activity-id="<?php bp_activity_id(); ?>" data-bp-timestamp="<?php bp_nouveau_activity_timestamp(); ?>" data-bp-activity="<?php ( function_exists('bp_nouveau_edit_activity_data') ) ? bp_nouveau_edit_activity_data() : ''; ?>">
	...
	<div class="bp-activity-head">
		...
		<div class="activity-header test-action">
			<?php 
			
			$user_id = bp_get_activity_user_id();
			$current_group_id = bp_get_current_group_id();
			$sub_roles = get_user_meta($user_id, "user_role", true );
			$is_user_admin = groups_is_user_admin($user_id, $current_group_id );

			if($sub_roles == "teacher"){
				?><div class="is-teacher-reply">Teacher’s Post</div><?php
			}else{
				if(!empty($is_user_admin)){
					?><div class="is-teacher-reply">Admin's Post</div><?php
				}
			}
