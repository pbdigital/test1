...
<div id="cover-image-container" class="<?php echo esc_attr( bb_platform_group_header_style() ); ?>">
	...
	$terms = wp_get_object_terms($ld_group_id,"ld_group_tag");
	$terms_arr = [];
	foreach($terms as $t) $terms_arr[] = $t->slug;

	if (filter_var( get_post_meta($ld_group_id, "class_cover_photo", true) , FILTER_VALIDATE_URL)) {
		$group_cover_image = get_post_meta($ld_group_id, "class_cover_photo", true);
	} else {
		$group_cover_image = wp_get_attachment_url($class_cover_photo);
	}

	$group_avatar_photo = get_post_meta($ld_group_id,"class_avatar",true);
	$is_custom_group_avatar = false;
	if(!empty($group_avatar_photo)){
		$is_custom_group_avatar = true;
		if (filter_var( get_post_meta($ld_group_id, "class_avatar", true) , FILTER_VALIDATE_URL)) {
			$group_avatar = get_post_meta($ld_group_id, "class_avatar", true);
		} else {
			$group_avatar = wp_get_attachment_url($group_avatar_photo);
		}
	}
	...
	<div testfeb16 id="header-cover-image" class="<?php echo esc_attr( 'cover-' . $group_cover_height . ' width-' . $group_cover_width . $has_cover_image_position . $has_cover_image . $has_default_cover ); ?>">
		<?php
	...
	<div id="item-header-cover-image" class="item-header-wrap <?php echo esc_attr( bp_disable_group_cover_image_uploads() ? 'bb-disable-cover-img' : 'bb-enable-cover-img' ); ?>">
		<?php if ( ! bp_disable_group_avatar_uploads() ) : ?>
			<div id="item-header-avatar">
				...
				<?php } ?>
				<?php 
				if(!$is_custom_group_avatar){
					bp_group_avatar();
				}else{
					?><img src="<?=$group_avatar."?".mt_rand()?>" class="avatar  avatar- photo" width="" height="" alt=""><?php
				}
				// ?>
			...
			<div id="item-header-content">
				...
				<div class="flex align-items-center bp-group-title-wrap">
					<h2 class="bb-bp-group-title">
						<?php echo wp_kses_post( bp_get_group_name() ); 
						if(in_array("institute-family-group", $terms_arr)){
							echo " Family";
						}
						?>
					</h2>
