...
//GROUP BY u.ID ";
		...
		// PBD: custom filter to allow Institute parents to view their students
		/*if(\Safar\SafarUser::is_user_institute_parent()){
			$parent_school_details = \Safar\SafarSchool::get_school_details([]);
			$all_user_ids_rearranged = [];
            foreach($parent_school_details->data["students"] as $student){
				$all_user_ids_rearranged[ (int) $student->data->ID ] = (int) $student->data->ID;
            }
	...
	public static function get_groups_list() {
		...
		$group_ids = false;
		// Modify custom query to restrict data to group leaders available data
		if ( 'group_leader' === self::get_user_role() || \Safar\SafarUser::is_user_institute_parent()) {
