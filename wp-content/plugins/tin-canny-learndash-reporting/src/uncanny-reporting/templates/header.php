...
-attendance (when avaialble)

remove course report for parents
...
$tabs[] = (object) [
	'id'   => 'tin-can',
	'href' => '#tin-can',
	'name' => __( 'Activity Report', 'uncanny-learndash-reporting' )
];
...
$tabs[] = (object) [
	'id'   => 'courseReportTab',
	'href' => '#courseReportTab',
	'name' => sprintf( _x( '%s Report', '%s is the "Course" label', 'uncanny-learndash-reporting' ), $course_label )
];
}


// Add "Tin Can Report" only in the admin section
if ( $tincanny_settings[ 'tinCanActivation' ] == '1' ){

	
	
	
	if ( ! is_admin() && ( ! isset( $tincanny_settings['enableTinCanReportFrontEnd'] ) || 1 != $tincanny_settings['enableTinCanReportFrontEnd'] ) ) {
		array_pop($tabs );
	}
	...
	$tabs[] = (object) [
		'id'   => 'xapi-tincan',
		'href' => '#xapi-tincan',
		'name' => __( 'Practice Report', 'uncanny-learndash-reporting' )
	];*/
	if ( ! is_admin() && ( ! isset( $tincanny_settings['enablexapiReportFrontEnd'] ) || 1 != $tincanny_settings['enablexapiReportFrontEnd'] ) ) {
		array_pop($tabs );
	}
}

// Default selected
$tab_active = 'userReportTab';
...
if ( is_admin() ){
	$css_classes[] = 'tclr-header--admin';
}
else {
	$css_classes[] = 'tclr-header--frontend';
}

?>

<div class="tclr-header <?php echo implode( ' ', $css_classes ); ?>">
    ...
    <nav class="nav-tab-wrapper uo-admin-reporting-tabs" testaaa data-tabgroup="first-tab-group" data-tab_on_load="<?php echo $tab_active; ?>">
    	<div class="tclr-admin-nav">
	    	<div class="tclr-admin-nav-items">
				<?php foreach ( $tabs as $tab ){ ?>
		    		...
		    		<a href="<?php echo $tab->href; ?>" class="nav-tab <?php echo implode( ' ', $css_classes ); ?>" data-tab_id="<?php echo $tab->href; ?>">
			            <?php echo $tab->name; ?>
			        </a>
