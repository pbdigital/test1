...
class Admin extends \UCTINCAN\Database {
	...
	private function get_query_string( $columns = 'id' ) {
		...
		if ( ! empty( $this->filters['actor'] ) ) {
			$where[] = "( user.`user_nicename` LIKE '%{$this->filters[ 'actor' ]}%' 
			OR user.`user_email` LIKE '%{$this->filters[ 'actor' ]}%' 
			OR user.`display_name` LIKE '%{$this->filters[ 'actor' ]}%' 
			OR usermeta.`meta_value` LIKE '{$this->filters[ 'actor' ]}%'
			)";
		}
		...
		$query = sprintf( "SELECT %s
			...
			LEFT OUTER JOIN %s usermeta ON ( usermeta.user_id  = reporting.user_id AND ( usermeta.meta_key='first_name' OR usermeta.meta_key='last_name' ) )
	...
	public function get_data( $per_page = 0, $mode = '' ) {
		...
		if ( $mode == 'csv' ) {
			...
			$query = $this->get_query_string( 'reporting.*, xgrouping.post_title as group_name, course.post_title as course_name, lesson.post_title as lesson_name, 
			
				user.display_name as user_name, 
				
				( SELECT meta_value FROM '.$wpdb->usermeta.' WHERE meta_key="first_name"  AND user_id=user.ID LIMIT 1 ) as first_name,
				( SELECT meta_value FROM '.$wpdb->usermeta.' WHERE meta_key="last_name"  AND user_id=user.ID LIMIT 1 ) as last_name
	...
	private function get_xapi_query_string( $columns = 'id' ) {
		...
		if ( ! empty( $this->filters['actor'] ) ) {
			$where[] = "( user.`user_nicename` LIKE '%{$this->filters[ 'actor' ]}%' 
							...
							OR ( 
								concat_ws(' ',
									(SELECT meta_value FROM ".$wpdb->usermeta." WHERE meta_key='first_name'  AND user_id=user.ID LIMIT 1 ),
									( SELECT meta_value FROM ".$wpdb->usermeta." WHERE meta_key='last_name'  AND user_id=user.ID LIMIT 1 )
								) LIKE '%{$this->filters[ 'actor' ]}%' 
							)
	...
	public function get_xapi_data( $per_page = 0, $mode = '' ) {
		...
		if ( $mode == 'csv-xapi' ) {
			...
			$query = $this->get_xapi_query_string( 'reporting.*, xgrouping.post_title as group_name, course.post_title as course_name, lesson.post_title as lesson_name, 
