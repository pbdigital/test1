				...
				<div class="uo-tclr-admin__content">
					...
					*/
					...
					<section id="first-tab-group" class="uo-admin-reporting-tabgroup">
						...
						<?php include self::get_part( 'login-report-tab.php' ); ?>

						<?php include self::get_part( 'practice-report.php' ); ?>

						<?php include self::get_part( 'attendance-report.php' ); ?>
						...
						<?php if ( is_admin() ) { ?>
							...
							<div class="uo-admin-reporting-tab-single" id="xapi-tincan"
								...
								<span test333 style="display:none"></span>
							...
							if ( isset( $tincanny_settings['enableTinCanReportFrontEnd'] ) ) {
								if ( $tincanny_settings['enableTinCanReportFrontEnd'] == 1 ) {
									...
									<div class="uo-admin-reporting-tab-single" id="tin-can"
										...
										<div class="reporting-datatable__table" test222>
											<?php self::show_tincan_list_table( 'tin-can' ); ?>
										</div>
							...
							if ( isset( $tincanny_settings['enablexapiReportFrontEnd'] ) ) {
								if ( $tincanny_settings['enablexapiReportFrontEnd'] == 1 ) {
									...
									<div class="uo-admin-reporting-tab-single" id="xapi-tincan"
										...
										<div class="reporting-datatable__table" test111>
											<?php self::show_tincan_list_table( 'xapi-tincan' ); ?>
										</div>
									...
									if(isset($_REQUEST["tab"])){
										if( $_REQUEST["tab"] == "xapi-tincan"){
										?>
										
										<script type="text/javascript">
											jQuery(document).ready( $ => {
												$( ".tablenav.bottom" ).append(`
												<div class="custom-xapi-loader" style="display:flex;align-items:center;position:absolute;justify-content:center;padding:30px;text-align:center;position: absolute;top:0px;z-index: 1;width: 100%;">
													...
													<circle cx="84" cy="50" r="10" fill="#5d53c0">
														...
														<animate attributeName="fill" repeatCount="indefinite" dur="2.564102564102564s" calcMode="discrete" keyTimes="0;0.25;0.5;0.75;1" values="#5d53c0;#5d53c0;#5d53c0;#5d53c0;#5d53c0" begin="0s"></animate>
													</circle><circle cx="16" cy="50" r="10" fill="#5d53c0">
													...
													</circle><circle cx="50" cy="50" r="10" fill="#5d53c0">
													...
													</circle><circle cx="84" cy="50" r="10" fill="#5d53c0">
												...
												let redrawTimeInterval = "";
												redrawTincannyXapi = () => {
													var allTables = $.fn.dataTable.tables();
													var desiredLength = <?=self::$tincan_opt_per_pages?>;

													for (var i = 0; i < allTables.length; i++) {
														$(allTables[i]).DataTable().page.len(desiredLength).draw();

														console.log("redraw datatable xapi", desiredLength)
													}
												...
												setTimeout( () => {
													...
													redrawTimeInterval = setInterval(() => {
														if( redrawTincannyXapi() ){
															clearInterval(redrawTimeInterval);
															$(".custom-xapi-loader").remove();
														}
													}, 500);
												}, 2000)
												...
												let ellipsisLoader = `
												<?xml version="1.0" encoding="utf-8"?>
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin: auto; background: none; display: block; shape-rendering: auto; position:absolute" width="41px"  viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
<circle cx="84" cy="50" r="10" fill="#ffffff">
    <animate attributeName="r" repeatCount="indefinite" dur="0.641025641025641s" calcMode="spline" keyTimes="0;1" values="10;0" keySplines="0 0.5 0.5 1" begin="0s"></animate>
    <animate attributeName="fill" repeatCount="indefinite" dur="2.564102564102564s" calcMode="discrete" keyTimes="0;0.25;0.5;0.75;1" values="#ffffff;#ffffff;#ffffff;#ffffff;#ffffff" begin="0s"></animate>
  ...
  <animate attributeName="cx" repeatCount="indefinite" dur="2.564102564102564s" calcMode="spline" keyTimes="0;0.25;0.5;0.75;1" values="16;16;16;50;84" keySplines="0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1" begin="-1.923076923076923s"></animate>
												...
												$(document).on("submit","#xapi-filters-top", e => {
													$("#xapi-filters-top #do_tcx_filter").html(ellipsisLoader)

													// Get the input element
													var inputElement = document.querySelector("#xapi-filters-top #do_tcx_filter");

													// Create a new button element
													var buttonElement = document.createElement("button");

													// Set the button text
													$(buttonElement).html(ellipsisLoader).addClass("button-primary tclr-reporting-button");
	...
	*/
	...
	public static function patchData() {
		...
		foreach ( $data as &$row ) {
			...
			$row['full-name'] = $row["first_name"]." ".$row["last_name"];
			$row['lesson'] = "<a href='".get_permalink($row["lesson_id"])."'>".$lesson->post_title."</a>";
			...
			$row['result']    = '<span class="tclr-reporting-datatable__no-wrap" tincannyreport>' . $result . '</span>';
	...
	public static function patchData_xapi() {
		...
		foreach ( $data as &$row ) {
			...
			if ( in_array( $lesson->post_type, $tincan_post_types ) ) {
				$group_link = admin_url( "post.php?post={$row[ 'group_id' ]}&action=edit" );
				$group_name = $row['group_name'];
				$group      = sprintf( '<a href="%s"  onclick="return false;">%s</a>', $group_link, $group_name );
			...
			$row["full-name"] = sprintf( '<a href="%s">%s</a>', admin_url( "user-edit.php?user_id={$row[ 'user_id' ]}" ), $row["fullname"] );
			...
			//$course_link = admin_url( "post.php?post={$row[ 'course_id' ]}&action=edit" );
			$row["lesson"] = "<a href='".admin_url( "post.php?post={$row[ 'lesson_id' ]}&action=edit" )."'>".$row["lesson_name"]."</a>";
	...
	private static function ExtraTableNavBottom() {
		...
		<div id="tincan-filters-export" testaaa>
